...
class UsageTreeProvider {
     ...
     */
    getDateGroupTurns(category) {
        const history = this.getHistory();
        const turns = [];
        for (const turn of history) {
            if (this.getDateCategory(turn.startTime) !== category)
                continue;
    ...
    getRootItems() {
        ...
        // 空状态引导
        if (history.length === 0) {
            const emptyItem = new UsageItem('还没有对话记录', vscode.TreeItemCollapsibleState.None, 'detail');
        ...
        if (todaySummary) {
            const summaryItem = new UsageItem('今日汇总', vscode.TreeItemCollapsibleState.Collapsed, 'summary', undefined, undefined, todaySummary);
        ...
        for (const cat of categories) {
            ...
            if (count === 0)
                continue;
            const dateItem = new UsageItem(cat, vscode.TreeItemCollapsibleState.Expanded, 'date', cat);
...
class UsageItem extends vscode.TreeItem {
    ...
    turnData;
    summaryData;
    constructor(label, collapsibleState, type, date, turnData, summaryData) {
