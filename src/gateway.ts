...
/**
 * 一次「对话回合」的统计记录
 * 注意：一个回合可能包含多次 API 请求（比如工具调用后继续生成），
 *      这些请求的 token 和费用会被累加到一起。
 */
interface TurnData {
    startTime: string;          // 回合开始时间（ISO 格式）
    userPrompt: string;         // 用户提问的摘要文本
    requestCount: number;       // 这个回合里总共发了几次 API 请求
    uncachedPromptTokens: number; // 输入 token 中「未命中缓存」的部分
    cachedTokens: number;       // 输入 token 中「命中缓存」的部分
    completionTokens: number;   // 模型输出的 token 数
    totalCost: number;          // 这个回合累计的费用
}

/**
 * 全局汇总状态（会持久化到 summary.json）
 * 这个汇总在网关进程重启后会被恢复，保证数据不丢失。
 */
interface SummaryState {
    totalTokens: number;        // 历史累计总 token 数
    totalCost: number;          // 历史累计总费用
    officialBalance?: string;   // DeepSeek 官方账户余额（通过 /user/balance 获取）
    apiKey: string;             // 从请求头里自动提取的 API Key
    lastUpdatedAt: number;      // 最后一次更新的时间戳
}

/**
 * DeepSeek API 返回的 usage 对象结构
 * 不同模型可能有细微差别，所以字段都设为可选。
 */
interface UsagePayload {
    ...
    prompt_tokens_details?: {
        cached_tokens?: number;             // 命中缓存的输入 token（OpenAI 格式）
    };
    prompt_cache_hit_tokens?: number;       // 命中缓存的输入 token（DeepSeek 格式）
}
...
let currentTurn: TurnData | null = null;         // 当前正在进行的对话回合
let turnFlushTimer: NodeJS.Timeout | null = null; // 延迟写入定时器（等 8 秒无新请求就写入文件）
...
/** 返回一个 JSON 响应 */
function writeJson(res: http.ServerResponse, statusCode: number, data: any) {
 ...
 */
function buildTargetUrl(reqUrl: string | undefined): URL {
    const base = new URL(targetApiUrl);
    if (!reqUrl) {
        return base;
    }
    // 如果已经是完整 URL 就直接用
 ...
 */
function buildUpstreamHeaders(req: http.IncomingMessage, bodyBuffer: Buffer, targetUrl: URL): http.OutgoingHttpHeaders {
 ...
 */
function cleanPromptCandidate(content: string): string {
    return content
 ...
 */
function isLikelyContextNoise(text: string): boolean {
    if (!text) return true;
 ...
 *   3. 实在找不到再退回完整清洗文本
 */
function extractActualUserPrompt(content: string): string {
 ...
 *   4. 截断到 50 个字符并补上 "..."
 */
function parseUserPromptSnippet(bodyBuffer: Buffer): string {
    try {
 ...
 */
function extractUsageFromSSE(text: string): UsagePayload | null {
 ...
 */
function extractUsageFromResponse(contentType: string, text: string): UsagePayload | null {
    if (!text) return null;
 ...
 *        + (输出 token / 1M)             × 输出单价
 */
function updateTurn(usage: UsagePayload, userPromptSnippet: string) {
    ...
    const requestCost =
        (uncachedPromptTokens / 1000000) * inputPricePerMillion +
 ...
 */
function fetchOfficialBalance(): Promise<string | null> {
            ...
            res.on('end', () => {
                try {
                    ...
                    if (json.is_available && Array.isArray(json.balance_infos) && json.balance_infos.length > 0) {
                        const cnyInfo = json.balance_infos.find((item: any) => item.currency === 'CNY') || json.balance_infos[0];
 ...
 */
async function handleControlRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string): Promise<boolean> {
    // 健康检查：插件用来判断网关是否在线
