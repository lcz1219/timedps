...
function startProxyServer(context) {
    ...
    proxyServer = http.createServer((req, res) => {
        try {
            ...
            const targetUrl = new url_1.URL(req.url || '', targetUrlStr);
            // 配置请求转发选项
            const options = {
            ...
            const proxyReq = https.request(options, (proxyRes) => {
                ...
                let buffer = '';
                let requestUsage = null; // 记录最后一次提取到的完整 usage 对象
...
function handleUsage(context, usage, userPromptSnippet) {
    ...
    const config = vscode.workspace.getConfiguration('aiUsageTracker');
    const inputPrice = config.get('inputPricePerMillion', 1.0);
    const outputPrice = config.get('outputPricePerMillion', 2.0);
